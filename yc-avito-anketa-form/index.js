...
//   GOOGLE_SERVICE_ACCOUNT_KEY  — весь JSON-ключ сервисного аккаунта, в одну строку (тот же, что у Анны Дейнеги / Светланы Идрисовой)
//   SPREADSHEET_ID              — ID таблицы, куда сейчас пишет анкета (из её ссылки, между /d/ и /edit)
//   SHEET_NAME                  — название листа, необязательно, по умолчанию "АВИТО"

// Порядок колонок A–M — тот же, в котором анкета отправляет поля в Apps Script,
...
async function appendToSheet(fields) {
  ...
  const sheetName = process.env.SHEET_NAME || 'АВИТО';
